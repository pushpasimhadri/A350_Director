 ...
 *Description   This file contains functions to test interfaces including UART,SPI,QSPI,I2C,DDR4,Ethernet 
 		and access the registers to read and write.
 *
 *@version      1.0
 *
...
int main() {
	...
	while (1) {
		...
		if(scanf("%d", &Menu) !=1 || Menu < 1 || Menu > QUIT) {
			printf("Invalid input. Please enter a valid option (1-7).\n");
			while('\n' != getchar());
			continue;
		}
		...
		switch(Menu) {
			case 1:
				...
				// Read and validate the user's sub-option selection
				if(scanf("%d", &Choice) !=1 || Choice < 1 || Choice > QUIT) {
					printf("Invalid input. Please enter a valid option(1-7).\n");
					while('\n' != getchar());
					continue;
				}
			...
			case 6:
				{
					...
					// Read and validate the user's operation selection
					if(scanf("%d", &obj_mem.operation) !=1 || obj_mem.operation < 1 || obj_mem.operation > 6) {
						printf("Invalid input. Please enter valid option(1-6).\n");
						while('\n' != getchar());
						continue;
					}
					...
					switch(obj_mem.operation) {
						...
						case 1:
							...
							printf(" Pass Address \n");
							// Read the memory address for 8-bit read operation
							scanf("%x", &obj_mem.address);
							break;
						case 2:
							printf(" 16 bit Read Selected \n");
							printf(" Pass Address \n");
							// Read the memory address for 16-bit read operation
							scanf("%x", &obj_mem.address);
							break;
						case 3:
							printf(" 32 bit Read Selected \n");
							printf(" Pass Address \n");
							// Read the memory address for 32-bit read operation
							scanf("%x", &obj_mem.address);
							break;
						case 4:
							printf(" 8 bit Write Selected \n");
							printf(" Pass Address \n");
							// Read the memory address for 8-bit write operation
							scanf("%x", &obj_mem.address);
							printf(" Pass Value \n");
							// Read the value to be written for 8-bit write operation
							scanf("%x", &obj_mem.value);
							break;
						case 5:
							printf(" 16 bit Write Selected \n");
							printf(" Pass Address \n");
							// Read the memory address for 16-bit write operation
							scanf("%x", &obj_mem.address);
							printf(" Pass Value \n");
							// Read the value to be written for 16-bit write operation
							scanf("%x", &obj_mem.value);
							break;
						case 6:
							printf(" 32 bit Write Selected \n");
							printf(" Pass Address \n");
							// Read the memory address for 32-bit write operation
							scanf("%x", &obj_mem.address);
							printf(" Pass Value \n");
							// Read the value to be written for 32-bit write operation
							scanf("%x", &obj_mem.value);
							break;
						default:
							// Display a message for wrong entry by the user
